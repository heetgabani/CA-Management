...
function FieldRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-3 gap-4 items-start py-4" style={{ borderBottom: '1px solid #F1F5F9' }}>
      <label className="text-sm font-medium pt-2" style={{ color: '#1F2937' }}>{label}</label>
      ...
      <div className="mt-6">
        {tab === 'firm' && (
          <div className="bg-white rounded-2xl border p-6" style={{ borderColor: '#E2E8F0' }}>
            <h3 className="text-sm font-semibold mb-1" style={{ color: '#1F2937' }}>Firm Profile</h3>
            <p className="text-xs mb-6" style={{ color: '#64748B' }}>Update your firm's basic information</p>

            {isLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="animate-spin w-5 h-5 rounded-full border-2 border-[#4F46E5] border-t-transparent" />
              </div>
            ) : (
              <form onSubmit={handleSubmit(d => updateFirm.mutate(d))}>
                ...
                <div className="grid grid-cols-3 gap-4 items-start py-4">
                  <label className="text-sm font-medium pt-2" style={{ color: '#1F2937' }}>City / State / PIN</label>
        ...
        {tab === 'branches' && (
          <div className="bg-white rounded-2xl border p-6" style={{ borderColor: '#E2E8F0' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold" style={{ color: '#1F2937' }}>Branches</h3>
        ...
        {tab === 'roles' && (
          <div className="bg-white rounded-2xl border p-6" style={{ borderColor: '#E2E8F0' }}>
            <h3 className="text-sm font-semibold mb-4" style={{ color: '#1F2937' }}>Role Permissions</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr style={{ borderBottom: '1px solid #E2E8F0' }}>
                    <th className="text-left py-2 px-3 font-medium" style={{ color: '#64748B' }}>Permission</th>
                    {['Owner', 'Partner', 'Manager', 'Accountant', 'Executive', 'Intern'].map(r => (
                      <th key={r} className="text-center py-2 px-3 font-medium" style={{ color: '#64748B' }}>{r}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={String(perm)} style={{ borderBottom: '1px solid #F1F5F9' }}>
                      <td className="py-2.5 px-3 font-medium" style={{ color: '#1F2937' }}>{perm}</td>
                      {vals.map((v, i) => (
                        <td key={i} className="py-2.5 px-3 text-center">
                          <span style={{ color: v ? '#10B981' : '#CBD5E1' }}>{v ? '✓' : '—'}</span>
                        </td>
                      ))}
                    </tr>
                  ))}
        ...
        {tab === 'notifications' && (
          <div className="bg-white rounded-2xl border p-6 space-y-4" style={{ borderColor: '#E2E8F0' }}>
            <h3 className="text-sm font-semibold" style={{ color: '#1F2937' }}>Notification Preferences</h3>
            {[
              ...
              <div key={key} className="flex items-center justify-between py-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
                ...
                <label className="relative inline-flex cursor-pointer">
                  ...
                  <div className="w-9 h-5 rounded-full peer-checked:bg-[#4F46E5] bg-[#E2E8F0] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4" />
                </label>
              </div>
            ))}
          </div>
        ...
        {tab === 'security' && (
          <div className="bg-white rounded-2xl border p-6 space-y-4" style={{ borderColor: '#E2E8F0' }}>
            <h3 className="text-sm font-semibold" style={{ color: '#1F2937' }}>Security Settings</h3>
            <div className="py-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium" style={{ color: '#1F2937' }}>Two-Factor Authentication</p>
            ...
            <div className="py-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: '#64748B' }}>Update your account password</p>
                </div>
            ...
            <div className="py-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium" style={{ color: '#1F2937' }}>Active Sessions</p>
                  <p className="text-xs mt-0.5" style={{ color: '#64748B' }}>View and revoke active login sessions</p>
                </div>
