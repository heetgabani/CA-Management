    ...
    <div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard title="Compliance Due"   value={stats.dueSoonCompliances ?? 0} icon={Shield}   iconBg="#FEF2F2" iconColor="#DC2626" />
          <StatCard title="Documents"        value={stats.totalDocuments  ?? 0} icon={FileText}    iconBg="#ECFDF5" iconColor="#15803D" />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {REPORTS.map(({ key, label, icon: Icon, desc }) => (
          <div key={key} className="bg-white rounded-2xl border p-5" style={{ borderColor: '#E2E8F0' }}>
            <div className="flex items-start gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-[#F1F5F9] flex items-center justify-center">
                <Icon size={18} style={{ color: '#1F2937' }} />
              </div>
              <div>
