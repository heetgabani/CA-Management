  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (row) => row.assignee
        ? <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-[#E2E8F0] flex items-center justify-center text-xs font-medium text-[#1F2937]">
              {row.assignee.firstName?.[0]}{row.assignee.lastName?.[0]}
            </div>
            <span className="text-sm text-[#1F2937]">{row.assignee.firstName} {row.assignee.lastName}</span>
          </div>
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span className="text-sm" style={{ color: isOverdue ? '#EF4444' : '#1F2937' }}>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* View toggle */}
          <div className="flex border rounded-xl overflow-hidden" style={{ borderColor: '#E2E8F0' }}>
            {([['list', LayoutList], ['kanban', Kanban]] as const).map(([v, Icon]) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className="w-9 h-9 flex items-center justify-center transition-colors"
                style={{ backgroundColor: view === v ? '#4F46E5' : '#FFFFFF', color: view === v ? '#FFFFFF' : '#9CA3AF' }}
              >
                <Icon size={15} />
              </button>
            ))}
