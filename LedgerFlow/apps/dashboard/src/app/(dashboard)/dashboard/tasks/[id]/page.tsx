    ...
    <div className="max-w-4xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-lg font-bold" style={{ color: '#1F2937' }}>{task.title}</h1>
        </div>
              ...
              {tab === 'overview' && (
                <div className="space-y-4">
                  ...
                  {task.comments?.length > 0 && (
                    <div>
                      <p className="text-xs font-medium mb-2" style={{ color: '#64748B' }}>RECENT COMMENTS</p>
                      {task.comments.slice(0, 3).map((c: any) => (
                        <div key={c.id} className="flex gap-3 mb-3">
                          <div className="w-7 h-7 rounded-full bg-[#E2E8F0] flex items-center justify-center text-xs font-medium flex-shrink-0">
                            {c.user?.firstName?.[0]}
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-sm" style={{ color: '#1F2937' }}>{c.content}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div>
                  {task.attachments?.length ? (
                    task.attachments.map((a: any) => (
                      <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl border mb-2" style={{ borderColor: '#E2E8F0' }}>
                        <Paperclip size={14} style={{ color: '#9CA3AF' }} />
                        <span className="text-sm flex-1" style={{ color: '#1F2937' }}>{a.fileName}</span>
          ...
          <div className="bg-white rounded-2xl border p-4 space-y-4" style={{ borderColor: '#E2E8F0' }}>
            ...
            {task.priority && (
              <div className="flex items-start gap-3">
                <div className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 rounded-full" style={{ backgroundColor: priority.variant === 'danger' ? '#FEF2F2' : '#FEF9C3' }} />
                <div>
