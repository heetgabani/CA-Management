...
const ROLE_MAP: Record<string, any> = {
  FIRM_OWNER:  { variant: 'default', label: 'Firm Owner' },
  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-[#E2E8F0] flex items-center justify-center text-sm font-semibold flex-shrink-0" style={{ color: '#1F2937' }}>
            {row.firstName?.[0]}{row.lastName?.[0]}
          </div>
          <div>
            <p className="text-sm font-medium" style={{ color: '#1F2937' }}>{row.firstName} {row.lastName}</p>
    ...
    },
    {
      ...
      render: (row) => row.lastLoginAt
        ? <span className="text-sm" style={{ color: '#64748B' }}>{new Date(row.lastLoginAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
    ...
    <div>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: '#1F2937' }}>Role</label>
            <select
              value={inviteRole}
