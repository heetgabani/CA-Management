...
function OverviewTab({ client }: { client: any }) {
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="mb-6">
      <h4 className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: '#6B7280' }}>{title}</h4>
  ...
  const Field = ({ label, value }: { label: string; value?: string | null }) => (
    <div className="p-3 rounded-xl" style={{ backgroundColor: '#F9FAFB' }}>
      ...
      <p className="text-sm font-medium" style={{ color: value ? '#111827' : '#D1D5DB' }}>{value || '—'}</p>
    </div>
  );
...
function DocumentsTab({ clientId }: { clientId: string }) {
  ...
  if (isLoading) return <div className="flex justify-center py-8"><div className="animate-spin w-5 h-5 rounded-full border-2 border-[#111827] border-t-transparent" /></div>;
  ...
  return (
    <div className="space-y-2">
      {documents.map((doc: any) => (
        <div key={doc.id} className="flex items-center gap-3 p-3 rounded-xl border transition-all" style={{ borderColor: '#E5E7EB' }}>
          <FileText size={16} style={{ color: '#6B7280' }} />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate" style={{ color: '#111827' }}>{doc.originalName}</p>
  ...
  return (
    <div className="space-y-2">
      {tasks.map((t: any) => {
        ...
        return (
          <a key={t.id} href={`/tasks/${t.id}`} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-[#F9FAFB] transition-all" style={{ borderColor: '#E5E7EB' }}>
            <CheckSquare size={15} style={{ color: '#6B7280' }} />
  ...
  return (
    <div className="space-y-2">
      {records.map((r: any) => {
        ...
        return (
          <a key={r.id} href={`/compliance/${r.id}`} className="flex items-center gap-3 p-3 rounded-xl border hover:bg-[#F9FAFB] transition-all" style={{ borderColor: '#E5E7EB' }}>
            <Shield size={15} style={{ color: '#6B7280' }} />
            <div className="flex-1">
              <p className="text-sm font-medium" style={{ color: '#111827' }}>{r.title}</p>
      ...
      {activities.map((a: any, i: number) => (
        <div key={i} className="flex gap-3">
          <div className="w-2 flex flex-col items-center">
            <div className="w-2 h-2 rounded-full mt-1.5 flex-shrink-0" style={{ backgroundColor: '#D1D5DB' }} />
            {i < activities.length - 1 && <div className="flex-1 w-px mt-1" style={{ backgroundColor: '#F3F4F6' }} />}
          </div>
          <div className="pb-3 flex-1">
            <p className="text-sm" style={{ color: '#374151' }}>{a.description}</p>
    ...
    <div className="max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h1 className="text-lg font-bold truncate" style={{ color: '#111827' }}>{client.displayName}</h1>
        </div>
      ...
      {/* Header card */}
      <div className="bg-white rounded-2xl border p-5 mb-5 flex items-start gap-5" style={{ borderColor: '#E5E7EB' }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-base font-bold" style={{ color: '#111827' }}>{client.displayName}</h2>
            <Badge
      ...
      <div className="mt-5 bg-white rounded-2xl border p-5" style={{ borderColor: '#E5E7EB' }}>
