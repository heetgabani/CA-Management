...
const ACTION_COLORS: Record<string, any> = {
  ...
  EXPORT: { variant: 'purple',  label: 'Export' },
  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (row) => row.user
        ? <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-[#E5E7EB] flex items-center justify-center text-xs font-semibold">{row.user.firstName?.[0]}</div>
            <div>
              <p className="text-sm font-medium" style={{ color: '#111827' }}>{row.user.firstName} {row.user.lastName}</p>
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <p className="text-sm" style={{ color: '#374151' }}>{row.entityType}</p>
    ...
    },
    {
      ...
      render: (row) => <span className="text-sm" style={{ color: '#374151' }}>{row.description || '—'}</span>,
    },
    {
      key: 'ipAddress', label: 'IP Address',
      render: (row) => <span className="text-xs font-mono" style={{ color: '#6B7280' }}>{row.ipAddress || '—'}</span>,
    },
    {
          ...
          {[{ key: '', label: 'All' }, ...Object.keys(ACTION_COLORS).map(k => ({ key: k, label: ACTION_COLORS[k].label }))].map(t => (
            <button
              ...
              style={{
                ...
                color: action === t.key ? '#111827' : '#6B7280',
