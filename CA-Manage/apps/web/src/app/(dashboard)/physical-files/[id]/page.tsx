    ...
    <div className="max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-lg font-bold font-mono" style={{ color: '#111827' }}>{file.fileNumber}</h1>
        </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl border p-5" style={{ borderColor: '#E5E7EB' }}>
            <h3 className="text-sm font-semibold mb-4" style={{ color: '#111827' }}>Movement History</h3>
            {file.movements?.length ? (
              <div className="space-y-3">
                {file.movements.map((m: any) => (
                  <div key={m.id} className="flex items-start gap-3 pb-3" style={{ borderBottom: '1px solid #F3F4F6' }}>
                    ...
                    <div className="flex-1">
                      ...
                      {m.fromLocation && <p className="text-xs mt-0.5" style={{ color: '#6B7280' }}>From: {m.fromLocation}</p>}
                      {m.toLocation && <p className="text-xs" style={{ color: '#6B7280' }}>To: {m.toLocation}</p>}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border p-4 space-y-4" style={{ borderColor: '#E5E7EB' }}>
            <h3 className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#6B7280' }}>File Details</h3>
            {[
            ...
            {file.client && (
              <div className="pt-3" style={{ borderTop: '1px solid #F3F4F6' }}>
                ...
                <a href={`/clients/${file.client.id}`} className="text-sm font-medium hover:underline" style={{ color: '#111827' }}>
      ...
      >
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: '#374151' }}>Issued To *</label>
            <input value={issuedTo} onChange={e => setIssuedTo(e.target.value)} placeholder="Staff name or client name"
              className="w-full h-9 px-3 text-sm rounded-xl border outline-none" style={{ borderColor: '#E5E7EB' }} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1" style={{ color: '#374151' }}>Reason</label>
