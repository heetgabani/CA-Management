  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (row) => (
        <div>
          <p className="font-medium text-sm font-mono" style={{ color: '#111827' }}>{row.fileNumber}</p>
    ...
    },
    {
      ...
      render: (row) => row.issuedToName
        ? <span className="text-sm" style={{ color: '#374151' }}>{row.issuedToName}</span>
        : <span className="text-sm" style={{ color: '#9CA3AF' }}>—</span>,
    },
    {
      key: 'issuedDate', label: 'Issued Date',
      render: (row) => row.issuedDate
        ? <span className="text-sm" style={{ color: '#374151' }}>{new Date(row.issuedDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
        : <span className="text-sm" style={{ color: '#9CA3AF' }}>—</span>,
    },
  ];
