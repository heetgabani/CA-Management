    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ backgroundColor: '#F3F4F6' }}>
          <ShieldCheck size={22} style={{ color: '#111827' }} />
        </div>
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: '#6B7280' }}>
            Enter the 4-digit code sent to <span className="font-medium" style={{ color: '#111827' }}>{email}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3 justify-center" onPaste={handlePaste}>
        {digits.map((digit, i) => (
          <input key={i} ref={inputRefs[i]} type="text" inputMode="numeric" maxLength={1} value={digit} autoFocus={i === 0}
            ...
            style={{ border: `2px solid ${digit ? '#111827' : '#E5E7EB'}`, borderRadius: '14px', backgroundColor: '#FFFFFF', color: '#111827' }}
            onFocus={e => (e.target.style.borderColor = '#111827')}
            onBlur={e => (e.target.style.borderColor = digit ? '#111827' : '#E5E7EB')} />
        ))}
      </div>
      <button type="button" disabled={otp.length < 4 || loading} onClick={() => submitOtp(otp)}
        ...
        style={{ backgroundColor: otp.length < 4 || loading ? '#9CA3AF' : '#111827', color: '#FFFFFF', borderRadius: '12px', cursor: otp.length < 4 || loading ? 'not-allowed' : 'pointer' }}>
      ...
      <div className="text-center text-sm" style={{ color: '#6B7280' }}>
        ...
        {countdown > 0
          ...
          : <button type="button" onClick={handleResend} disabled={resending} className="font-medium inline-flex items-center gap-1 hover:underline" style={{ color: '#111827' }}>
              {resending ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />} Resend OTP
            </button>}
      </div>
      <p className="text-center text-sm" style={{ color: '#6B7280' }}>
        <Link href="/login" className="font-medium hover:underline" style={{ color: '#111827' }}>Back to login</Link>
      </p>
    </div>
  );
}
